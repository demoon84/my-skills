  ...
  for (const [tool, targetPath] of Object.entries(targets)) {
    ...
    if (lstat) {
      try {
        ...
        } else if (lstat.isDirectory() && isCopiedSkill(skill, targetPath)) {
          matches = true;
          status = "copied";
        } else {
          status = "exists-other";
...
function isCopiedSkill(skill, targetPath) {
  const sourceSkillMd = skill.skillMdPath;
  ...
  return fs.readFileSync(sourceSkillMd, "utf8") === fs.readFileSync(targetSkillMd, "utf8");
}
