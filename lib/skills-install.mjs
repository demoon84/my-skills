...
export function generateManifest(tool, skills, {
  skillsConfig,
  scope = "global",
  cwd = process.cwd(),
  ...
  return buildUnifiedManifest(tool, relevantSkills, skillsConfig, scope, cwd, platform);
}
...
function resolveHookCommand(skillInstallPath, scriptRel, platform = process.platform) {
  const scriptPath = path.join(skillInstallPath, scriptRel);

  if (platform === "win32") {
    const powershellScriptPath = resolvePowerShellVariant(scriptPath);
    if (powershellScriptPath) {
      return formatPowerShellCommand(powershellScriptPath);
    }
  }
...
function resolvePowerShellVariant(scriptPath) {
  if (scriptPath.endsWith(".ps1") && fs.existsSync(scriptPath)) {
    return scriptPath;
  }

  if (scriptPath.endsWith(".sh")) {
    const powershellVariant = `${scriptPath.slice(0, -3)}.ps1`;
    if (fs.existsSync(powershellVariant)) {
      return powershellVariant;
    }
  }
...
function formatPowerShellCommand(scriptPath) {
  const powershellExe = "C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe";
  return `${quoteCommandArg(powershellExe)} -NoLogo -NoProfile -ExecutionPolicy Bypass -File ${quoteCommandArg(scriptPath)}`;
}
...
// Codex uses these lifecycle event names in the generated hook manifest.
function buildUnifiedManifest(tool, skills, skillsConfig, scope, cwd, platform) {
  const manifest = {
    ...
    for (const [event, scriptRel] of Object.entries(skill.hooks)) {
      ...
      buckets[eventKey].push({
        hooks: [
          {
            type: "command",
            command: resolveHookCommand(skillInstallPath, scriptRel, platform),
            timeout: 30
          }
        ]
      });
